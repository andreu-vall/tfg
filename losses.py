...
def peter_loss(loss_input, loss_output, text_criterion, rating_criterion, context_reg, text_reg, rating_reg):
    
    log_word_prob, log_context_dis, predicted_rating = loss_input
    target_text, real_rating = loss_output

    assert log_context_dis is not None

    ntokens = log_context_dis.shape[1] # espero que no pugui ser diferents formes...
